...
### 1. We need to set up a custom concurrency limit for them

Custom session creation rates are available **at no extra cost** -- they just need to know the target workload. For 2,000 concurrent streams, a custom rate ensures you can ramp up without waiting for auto-scaling to catch up.
